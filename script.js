...
// Modificar la función de inicio para móviles
document.addEventListener('DOMContentLoaded', () => {
    initParticles();
    initVoiceRecognition();
    initCustomization();
    ...
    // Añadir clase móvil al body si es necesario
    if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
        document.body.classList.add('is-mobile');
    }
    
    // Configurar eventos táctiles para los botones
    document.querySelectorAll('.controls button, .food-item, .rps-choice').forEach(button => {
...
// Mejorar el arrastre de comida para móviles
function startFoodDrag(food) {
    isDraggingFood = true;
    const foodCursor = document.getElementById('foodCursor');
    foodCursor.textContent = food;
...
function stopFoodDrag() {
    ...
    selectedFood = null;
    const foodCursor = document.getElementById('foodCursor');
    if (foodCursor) foodCursor.style.display = 'none';
    
    if (document.body.classList.contains('is-mobile')) {
        document.body.classList.remove('is-dragging-food');
    } else {
        document.body.style.cursor = '';
    }
}
